...
History:
    10/16/13 - Copied from personal repo to shared repo
'''
...
def writelines(lines, filename, mode='w'):
    '''
    Function: writelines
    Input:
    ...
    History:
        ...
        1/31/14 - adding mode to allow appends
    '''
    if mode != 'w':
        filedescriptor = openw(filename, mode)
    else:
...
def openw(filename='', mode='w', binary=False):
    '''
    Function: openw
    ...
    History:
        ...
        1/31/14 - adding mode to allow for appends
    '''
    if filename:        # if filename given
        ...
        if binary:
            descriptor = open(filename, mode + 'b') # open file for binary write
        else:
            descriptor = open(filename, mode)       # open file for write
